...
func main() {
	...
	for _, entry := range data[10:] {
		...
		pivotIndex := len(fromColumn) - num
		cratesInMotion := fromColumn[pivotIndex:]
		crates[command.To] = append(toColumn, cratesInMotion...)
		crates[command.From] = fromColumn[:pivotIndex]
	}
